    ...
    public class SpMapper_v3_00
    {
        ...
        public RouteConstraints Map(JourneyProfile journeyProfile, SegmentProfile[] segmentProfiles, ITrainCharacteristics trainCharacteristics)
        {
            ...
            foreach (var item in spList)
            {
                ...
                var gradientSegments = GetGradientProfile(sp.SP_Characteristics.Gradient, absPos, (float)sp.SP_Length);
        ...
        private static List<GradientSegment> GetGradientProfile(Gradient gradientItem, float absPos, float spLength)
        {
            if (gradientItem == null || gradientItem.GradientStart == null || gradientItem.GradientChange == null)
            {
                return new List<GradientSegment>();
            }

            var gradients = new List<GradientSegment>();
            var gradientChanges = gradientItem.GradientChange;

            gradients.Add(new GradientSegment(absPos, absPos + (float)gradientChanges[0].location, (float)gradientItem.GradientStart.gradientValue));
        ...
        private static List<CurveSegment> GetCurveProfile(Curves curveItem, float absPos, float spLength)
        {
            ...
            curves.Add(new CurveSegment(absPos, absPos + (float)curveChanges[0].location, curveItem.CurveStart.curveRadius));
            ...
            for (int i = 0; i < curveChanges.Length; i++)
            {
                ...
                {
                    curve = new CurveSegment(absPos + (float)change.location, absPos + (float)curveChanges[i + 1].location, change.curveRadius);
                }
                else
                {
                    curve = new CurveSegment(absPos + (float)change.location, absPos + spLength, change.curveRadius);
                }
                curves.Add(curve);
            }
            return curves;
